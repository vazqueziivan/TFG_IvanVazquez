...
public class RobotHelper {
    ...
    public boolean askToCloseIfFlapIsOpened() {
        ...
        if (isFlapOpened)
            say("Por favor, cierra primero mi solapa de carga");
        return isFlapOpened;
    }
    ...
    public Future<Void> say(final String text) {
        return SayBuilder.with(qiContext)
                .withText(text)
                .withLocale(new Locale(Language.SPANISH, Region.SPAIN))
    ...
    public Future<Void> mirarPersona(final Human human) {
        return EngageHumanBuilder.with(qiContext)
                .withHuman(human)
                .buildAsync().andThenCompose(mirar -> {
                    Log.d(TAG, "Seguir started");
                    return mirar.async().run();
                });
    }

    public Future<Void> seguirPersona(final Human human) {
        return ApproachHumanBuilder.with(qiContext)
                .withHuman(human)
                .buildAsync().andThenCompose(seguir -> {
                    Log.d(TAG, "Seguir started");
                    return seguir.async().run();
                });
    }

    public Future<PhraseSet> phraseSet(final String text) {
        return PhraseSetBuilder.with(qiContext)
                .withTexts("¿Dónde estamos?")
                .buildAsync();
    }

    public Future<ListenResult> listen(final Future<PhraseSet> phraseSet) {
        return ListenBuilder.with(qiContext)
                .withPhraseSet((PhraseSet) phraseSet)
                .buildAsync().andThenCompose(listen -> {
                    Log.d("COSITAS", "Listen started : " + phraseSet.toString());
