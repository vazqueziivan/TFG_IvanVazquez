...
public class DuchadoFragment extends Fragment {
...
//    private int hora;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container,
        ...
        int fragmentId = R.layout.fragment_duchado;
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        // Cambia el valor del booleano para saber si te has duchado en función de la respuesta
        listen.thenConsume(v -> {
            if (v.isDone()) {
                String frase = v.getValue().getHeardPhrase().getText().toString();
                Log.i("FRASE", frase);
                if (frase.equals("Si")) {
                    ma.robotHelper.say("Con razón hueles tan bien").thenConsume(a -> {
                        ma.duchado_b = true;
                        listen.cancel(true);
                        ma.setFragment(new HumanoEncontradoFragment(), false);
                    });
                } else if (frase.equals("No")) {
                    ma.robotHelper.say("Ya decía yo que olía un poco mal aquí").thenConsume(a -> {
                        ma.duchado_b = false;
                        listen.cancel(true);
                        ma.setFragment(new HumanoEncontradoFragment(), false);
                    });
