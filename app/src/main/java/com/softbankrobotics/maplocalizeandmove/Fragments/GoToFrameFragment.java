...
public class GoToFrameFragment extends Fragment {
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        view.findViewById(R.id.back_button).setOnClickListener((v) ->
                ma.setFragment(new PruebasFragment(), true));
    ...
    public void goToLocation(String location, OrientationPolicy orientationPolicy) {
        createGoToPopup();
        ma.runOnUiThread(() ->goto_text.setText("Vamos hasta " + location));
        ...
        ma.robotHelper.goToHelper.addOnFinishedMovingListener((goToStatus) -> {
            ...
            if (!location.equalsIgnoreCase("BaseCarga")) {
            ...
            ma.runOnUiThread(() -> {
                if (goToStatus == GoToHelper.GoToStatus.CANCELLED) {
                    ...
                    FutureUtils.wait(4, TimeUnit.SECONDS).thenConsume(aUselessFuture -> {
                        ...
                        if (goToStatus == GoToHelper.GoToStatus.FINISHED && location.equalsIgnoreCase("BaseCarga")) {
        ...
        ma.goToLocation(location, orientationPolicy, this);
    }
            ...
            ma.robotHelper.goToHelper.addOnFinishedMovingListener((goToStatus) -> {
                ...
                if (goToStatus == GoToHelper.GoToStatus.FINISHED && gotoRouteCounter < ma.locationsToGoTo.size()) {
                    ...
                    if (gotoRouteCounter == ma.locationsToGoTo.size() - 1) {
                        ma.goToLocation(ma.locationsToGoTo.get(gotoRouteCounter), OrientationPolicy.ALIGN_X, this);
                    } else {
                        ma.goToLocation(ma.locationsToGoTo.get(gotoRouteCounter), OrientationPolicy.FREE_ORIENTATION, this);
                    }
                } else {
                    ma.robotHelper.goToHelper.removeOnFinishedMovingListeners();
                    Log.d(TAG, "goToRoute: removeOnFinishedMovingListeners");
                    if (!ma.locationsToGoTo.get(gotoRouteCounter - 1).equalsIgnoreCase("BaseCarga")) {
                        ma.robotHelper.releaseAbilities();
                    }
                    ma.runOnUiThread(() -> {
                        if (goToStatus == GoToHelper.GoToStatus.CANCELLED) {
                            ...
                            FutureUtils.wait(4, TimeUnit.SECONDS).thenConsume(aUselessFuture -> {
                                ...
                                if (goToStatus == GoToHelper.GoToStatus.FINISHED && ma.locationsToGoTo.get(gotoRouteCounter - 1).equalsIgnoreCase("BaseCarga")) {
    ...
    private void displayLocations() {
        ...
        if (ma.savedLocations.isEmpty()) noLocationsToLoad();
            ...
            goToMapFrameButton.setOnClickListener((useless) -> {
                if (!ma.robotHelper.askToCloseIfFlapIsOpened()) {
                    goToLocation("Casa", OrientationPolicy.ALIGN_X);
                }
            });
    ...
    private void noLocationsToLoad() {
        ...
        close.setOnClickListener((v) -> {
            ...
            ma.setFragment(new AjustesFragment(), true);
        });
