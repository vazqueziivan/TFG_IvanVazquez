...
import com.softbankrobotics.maplocalizeandmove.Fragments.AjustesFragment;
import com.softbankrobotics.maplocalizeandmove.Fragments.PantallaInicioFragment;
import com.softbankrobotics.maplocalizeandmove.Fragments.PosicionesFragment;
...
public class MainActivity extends RobotActivity implements RobotLifecycleCallbacks, Serializable {
    ...
    public PhraseSet PH_puerta;
    public PhraseSet b;
    public Listen frases;
    public String destino = null;
    public Chat chat;
    private Locale locale = new Locale(Language.SPANISH, Region.SPAIN);
    public Human human = null;
//    public int pastilla;
//    public int comida;
//    public int ducha;
    public boolean comido_b = false;
    public boolean pastilla_b = false;
    public boolean duchado_b = false;
        ...
        if (goToRandomWasActivated || goToRouteWasActivated) {
            if (savedLocations.containsKey("ChargingStation")) {
                if (!robotHelper.goToHelper.isDocked(savedLocations.get("ChargingStation")).getValue()) {
                    resumeGoTo();
                } else runOnUiThread(() -> setFragment(new AjustesFragment(), false));
            } else resumeGoTo();
        } else {
            runOnUiThread(() -> setFragment(new PantallaInicioFragment(), false));
        }
    ...
    // Construye las frases que se escucharán posteriormente
    public void construyeListener()
    {
        List<Phrase> listaUbicaciones = new ArrayList<Phrase>();
        Phrase dondeEstamos = new Phrase("Donde estamos");
        Phrase sigueme = new Phrase("Sigueme");
        Phrase chat = new Phrase("Chat");
        Phrase tomar_pastilla = new Phrase("Me he tomado la pastilla");
        Phrase hambre = new Phrase("Tengo hambre");
        Phrase ducha = new Phrase("Deberia ducharme");
        Phrase Si = new Phrase("Si");
        Phrase No = new Phrase("No");
        Phrase Adios = new Phrase("Adios");
        listaUbicaciones.add(dondeEstamos);
        listaUbicaciones.add(sigueme);
        listaUbicaciones.add(chat);
        listaUbicaciones.add(tomar_pastilla);
        listaUbicaciones.add(hambre);
        listaUbicaciones.add(ducha);
        listaUbicaciones.add(Si);
        listaUbicaciones.add(No);
        listaUbicaciones.add(Adios);
        Iterator<String> iterator = savedLocations.keySet().iterator();
        while (iterator.hasNext()) {
            Phrase nuevaUbicacion =new Phrase("Vamos " + iterator.next());
            listaUbicaciones.add(nuevaUbicacion);
        }

        PhraseSetBuilder psb = PhraseSetBuilder.with(qiContext);
        psb.withPhrases(listaUbicaciones);
        Future<PhraseSet> a = psb.buildAsync();
        while (!a.isDone()) {
            try {
                Thread.sleep(100);
            }
        ...
        Future<Listen> c = ListenBuilder.with(qiContext)
                .withPhraseSet(b)
                .buildAsync();
    ...
    public void setFragment(Fragment fragment, Boolean back) {
        ...
        //previous = getSupportFragmentManager().findFragmentByTag(tag);
     ...
     */
    public void goToLocation(final String location, OrientationPolicy orientationPolicy, Object f) {
        Log.d(TAG, "goToLocation: " + location);
        robotHelper.say("Vamos hasta " + location);
        Log.d(TAG, "Fragment: " + f.hashCode());
        robotHelper.goToHelper.checkAndCancelCurrentGoto().thenConsume(aVoid -> {
            robotHelper.holdAbilities(true);

            if (location.equalsIgnoreCase("Casa")) {
                robotHelper.goToHelper.goToMapFrame(goToStraight, goToMaxSpeed, orientationPolicy);
            } else if (location.equalsIgnoreCase("BaseCarga")) {
    ...
    public void backupLocations() {
        ...
        runOnUiThread(() -> {
            PosicionesFragment saveLocationsFragment = (PosicionesFragment) getFragment();
...
//                    humano = GoToBuilder.with(qiContext)
//                            .withFrame(position)
